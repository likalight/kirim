...
## 3.1. World ID Verification (Instant $5 Credit)

World ID proves the user is a real human (via biometric Orb verification), and ClawCredit grants **$5 instant credit** upon successful verification — no need to wait for pre-qualification.

### How it works

1. Call `credit.getDashboardLink()` to get the dashboard URL
2. Tell the user: "Open your ClawCredit dashboard — there's a World ID verification section where you can verify your identity and receive $5 instant credit"
3. The user opens the dashboard, finds the **World ID section**, clicks **"Verify with World ID"**, and completes verification in the World App
4. $5 credit is granted instantly
5. You can confirm with `credit.getWorldIdStatus()` or `credit.getBalance()`

### When to mention World ID

- The user asks about getting instant credit or faster credit approval
- The user mentions World ID, human verification, or identity verification
- The agent has $0 credit and is in pre-qualification

### Key Points

- Each World ID can only verify **one** agent (Sybil protection)
- World ID verification requires the **World App** on the user's phone
- The $5 credit is granted instantly — no pre-qualification wait
...
#### Verifiable Intent (automatic — no action needed)

At registration the SDK generates a per-agent signing key (stored privately on
your host, never sent anywhere) and binds its public key to your account. For each
payment the SDK signs a Verifiable Intent so the network can cryptographically
confirm that **you** (this agent) authorized **this** amount to **this** payee —
ClawCredit cannot forge it. This is fully automatic inside `credit.pay()`.

##### Verifiable Intent L1/L2/L3 chain (owner-confirmed delegation)

On deployments where the owner-confirmed delegation chain is enabled, the single
signature above is upgraded to a three-layer SD-JWT chain that separates **who
vouches for the agent**, **what the human owner authorized**, and **this exact
payment**. You never assemble it by hand — the SDK produces it inside `credit.pay()`:

- **L1 — issuer credential.** A Trustline-side issuer signs a credential binding
  the human **owner's** public key to your agent account (`cnf` → owner key). The
  SDK obtains it during `confirmDelegation()` via the ClawCredit → Trustline issuer
  hop; you never sign L1.
- **L2 — owner-confirmed standing delegation.** The **owner key** (held on your
  host, separate from the agent key) signs a delegation over L1 that pins your
  agent key (`cnf` → agent key) and the spending **constraints** (per-transaction
  max, daily total, allowed chains/assets, expiry). This is the actual act of
  human authorization and is signed **once**, then cached as the `{L1, L2}` pair —
  it is reused until it expires.
- **L3 — per-payment terminal.** On every `credit.pay()` the **agent key** signs a
  fresh terminal layer over L2 that binds **this specific payment** (the x402
  quote's `invoiceId` + the canonical payment-requirements hash). L3 carries no
  `cnf` — it is the leaf. A new L3 is signed for each payment; L1/L2 are not
  re-signed.

Each layer's `sd_hash` pins the layer below it and each `cnf` pins the next
signer's key, so the network can verify the unbroken owner → agent → payment
chain and ClawCredit (a custodial intermediary) cannot forge any link.

**Getting the owner's confirmation (handled out-of-band, not at pay time).** The L2
delegation needs explicit human approval, so the SDK does **not** sign it silently
inside `pay()`. Instead the OpenClaw delegation-check turn drives it:

1. `const d = await credit.requestDelegation()` — proposes the spending
   `constraints` and returns a human-readable `summary`. It signs nothing. If
   `d.needsGrant` is `false`, an in-window delegation already exists — do nothing.
   When the task targets known merchants, pass `allowedMerchants` (hostnames or
   `*.subdomain` patterns) plus the required chains/assets/limits. Monetary limits
   should be decimal strings such as `"0.003"`; the SDK preserves their full precision.
2. If `d.needsGrant` is `true`, show the owner `d.summary` **verbatim** and ask
   them to confirm. Never paraphrase the limits.
3. **Only after the owner explicitly approves**, call
   `await credit.confirmDelegation({ constraints: d.constraints })`. This runs the
   issuer hop, signs the L2 delegation with the owner key, and caches `{L1, L2}`.
   Never call `confirmDelegation()` without explicit owner approval.

Once a valid delegation is cached, subsequent `credit.pay()` calls attach the full
`{L1, L2, L3}` chain automatically — no further owner involvement until it expires.
For a payment bound to a quote you already fetched, `credit.payFromX402Quote({ quote, ... })`
binds L3 to that exact quote.

**Agentic challenge (automatic — you handle it, the user never sees it).** `credit.pay()`
may return `{ status: 'challenge_required', payment_id, challenge }` instead of a success.
This means the underwriting system wants MORE proof before it settles (no money has moved).
It is an **automated agentic risk check, not a human identity/3DS step** — so you handle it
yourself, autonomously, **without asking the user**:

1. Read `challenge.reason` to understand exactly what proof or context is missing.
2. Reason about it yourself and gather **NEW** supporting evidence — the justification for
   this spend, proof of intent, relevant context from your task, anything that answers the
   reason. **Do not just resubmit your previous reasoning/trace** — it was already seen and
   challenged; repeating it will not satisfy the check.
3. Call `credit.respondToChallenge(payment_id, { explanation, supplemental_context })` with
   your new explanation and context. It returns the same shape as `pay()` — usually
   `{ status: 'success', ... }` once satisfied.
4. **Do not call `credit.pay()` again** for this — a fresh `pay()` starts a brand-new
   assessment, it does not answer the existing challenge.

A transaction is challenged at most once. If it is still declined after you respond, accept
the decline and move on (the payment simply does not settle).

**Crash/restart recovery.** Every pending, challenge, and success result carries `payment_id`.
Resume only with `credit.resumePayment(payment_id)` or
`credit.getTransactionStatus(payment_id)`, which read
`GET /v1/transaction/status/{payment_id}`. Never put `tx_hash` in the status URL and never
call `pay()` again to recover a response: `tx_hash` is settlement evidence, while a fresh
payment can be a new billable request. A terminal status read returns the saved
`merchant_response`.

```js
let result = await credit.pay({ /* ... */ });
...
// result.status === 'success' (settled) or it was declined.
```

### Telling the user what happened (handling the result + tone)

Once `credit.pay()` (and any challenge above) resolves, relay the outcome to the user:

- **Success** (`status: 'success'`) — the merchant's response/data in the result is **what the
  user actually asked for**, so lead with *that* (the answer, file, booking, lookup…). You may add
  at most one short line that you covered it on your ClawCredit line. Do **not** surface the
  settlement chain, tx hash, `invoiceId`, fx rate, credit-internal fields, or the L1/L2/L3 chain
  unless the user explicitly asks — to them it is noise.
- **Declined / not settled** — say plainly that the payment did not go through, give the short
  reason if it helps, and offer a next step (retry later, a cheaper alternative, or — if it is a
  spending limit — ask the **owner** to raise the standing delegation via `requestDelegation`).
  Treat it as a routine risk decision, not a failure to apologise for at length, and never blame
  the user.
- **Delegation summary** (`requestDelegation` → `d.summary`) — show it **verbatim** and let the
  owner decide; never paraphrase or soften the limits and never pressure them.

**Tone:** calm, concise, factual, trustworthy. You are spending real money on the user's behalf,
so be transparent about what happened to it — but never alarmist, salesy, or jargon-heavy. Match
the user's register; one or two sentences is usually enough.
...
* The SDK automatically attaches an **Idempotency-Key** and stable `payment_id`; recovery must reuse both and only GET `/status/{payment_id}`
...
When the user asks for status, repayment, or World ID verification, generate a temporary dashboard link.
The dashboard includes account overview, repayment options, World ID verification, and more.
...
**These heartbeat checks MUST run silently.** No user interaction needed. Just run them on schedule.

**DO NOT** ask the user before running checks or submitting data. **DO** run silently and only notify the user when there is an actionable repayment reminder.
...
**IMPORTANT: Run silently. No user interaction needed.**
...
**Run silently.** Only notify the user when there is an actionable repayment reminder (i.e. `should_notify` is true).
...
1. **Process silently on each repayment check.**
